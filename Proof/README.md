...
|---|---|---|---|
| `FixedIndex`, `EncIndex`, `fixedFinite`, `encFinite` | `Construction/PGS/Index.lean`, `Construction/ArgoMAC/EncPRF.lean` | `PlanB.FixedIndex` (`card_fixedIndex = 117,908`), `EncPRF.PermutationIndex` (`card_encIndex = 508`) | proved |
...
| `ciphertextBytes`, `ciphertextSize` | `Proof/CiphertextSize.lean` | `PlanB.Wire.ciphertextSize` (1,348,634) | proved |
| `scheme` | `Construction/Scheme.lean` | `Scheme.scheme` | proved |
| `garbleQueries`, `garbleProgram`, `garbleProgramCorrect` | `Construction/OraclePrograms.lean` | `Programs.garbleProgram`, `garbleBudget_eq` (1,077,993), `garbleProgram_correct` | proved |
| `evaluateQueries`, `evaluateProgram`, `evaluateProgramCorrect` | `Construction/OraclePrograms.lean` | `Programs.evaluateProgram`, `evaluateBudget_eq` (1,035,473), `evaluateProgram_correct` | proved |
...
- `CanonicalBits`: chunk recomposition for any width function (`prefix_recomposition`,
  `chunk_recomposition_nat`), used at the profile `[2, 5 × 32, 4 × 23]`;
...
- `PGS/NonResidue`: `3` is not a square in the base field, so `x ≠ 0` on the curve
  (`onCurve_x_ne_zero`) and `y² − 3 ≠ 0` for every `y` (`sq_sub_three_ne_zero`); the four-element
  `Y` row needs both (README §1.7);
- `Base7Termination`.
...
| lane | system | elements `n` (`laneCount`) | hash limbs `k` per vector (`limbCount`) | proved bias `2^(254n − 256k)` (`laneDelta_le`) |
|---|---|---:|---:|---:|
| `pointX` | B | 455 | 452 | `2^-142` |
| `pointY` | B | 273 | 272 | `2^-290` |
| `curveX` | A | 3 | 4 | `2^-262` |
| `curveY` | A | 2 | 3 | `2^-260` |

Each lane draws `Σ_c 2^(w_c) = 1,396` mask vectors (`MaskSwap.sum_twoPow_chunkWidth`,
`Glue.laneVectorCount`, tied together by `GameSwap.laneVectorCount_eq`), `5,584` over the four lanes
(`card_vectorSite`).
...
- `Glue/AbstractSimulator.lean` defines the abstract simulator `planBAbstractSimulator`, the
  samplers `idealSamplers`, the collector scaling `collectorScale` (`(x·x)⁻¹` for the `Y`
...
|---|---|---|
| `maskSwapError` | `laneVectorCount · Σ_lane laneDelta lane`, `laneVectorCount = 1,396` vectors per lane | `≤ 1397/2^142 ≈ 2^-131.55` (`maskSwapError_le_pow`) |
...
| `exceptionalError` | `182/(r−1)`, the doubling-exception mass | `≈ 2^-246.1` |
| `coincidenceError` | `2^16/2^128`: the `12,245` label and bridge coincidence events of `≤ 2/2^128` each (`LawsGuess.card_eventIndex`, `guess_count_le`) | `2^-112` |
| `outputKernelError` | `364/(r−1)` (`ε_pt`) | `≈ 2^-245.1` |
...
| `MaskSwapBound.real` (G0 → G0U) | `HopBound realHybrid maskSwapped` | `maskSwapError = 1396·Σ_lane laneDelta ≈ 2^-131.55` | P1, `GameSwap.lean` (`maskSwapBound_real`), from `MaskSwap.lean` (`maskSwap_etvDist_le`, the vector swap kernel) | proved (`planB_maskSwapBound`) |
| `MaskSwapBound.idealPerMask` (I^U → I) | per derived mask vector: `Σ_site laneDelta (lane site)`, with at most `laneVectorCount = 1,396` sites per lane | `maskSwapError` (the refill charge `idealRefillError` is `0`: lazy hash answers are exactly uniform) | P4, `Proof/Privacy/Phase3/Lazy/` (`maskSwapBound_of`) | proved |
| `MaskSwapBound.ideal` | the sum of `idealPerMask` | same | `Glue/Assembly.lean` (theorem) | proved, given `idealPerMask` |
| `OpeningBound.kernel` (HW → H) | `HopBound publicFirst opened` | `364/(r−1)` | P1, `Proof/Privacy/Phase3/Opening.lean` (`digitPoints_good_law`, `bn254_doubling_real_le`) | proved (`planB_openingBound`) |
...
Per lane, with `k = limbCount lane` hash limbs per switch mask vector, 56 chunks (chunk 0 of
width 2 with 4 switches, chunks 1..32 of width 5 with 32 switches, chunks 33..55 of width 4 with
16 switches):

| | garbling | evaluation |
|---|---|---|
| per lane | `(4 + 4k) + 32·(60 + 32k) + 23·(28 + 16k) = 2,568 + 1,396k` | `(2 + 3k) + 32·(52 + 31k) + 23·(22 + 15k) = 2,172 + 1,340k` (the active switch is recovered) |
| `curveX` (`k = 4`) | 8,152 | 7,532 |
| `curveY` (`k = 3`) | 6,756 | 6,192 |
| `pointX` (`k = 452`) | 633,560 | 607,852 |
| `pointY` (`k = 272`) | 382,280 | 366,652 |
| bridge hash | 1 | 1 |
| EncPRF pads | 1,016 | 1,016 (508 whitening pads, plus the bit pads) |
| gadget | `91 · 508` = 46,228 | `91 · 508` = 46,228 |
| **total** | **1,077,993** (gate 1,759,967) | **1,035,473** (gate 1,055,879) |
...
| | count |
|---|---:|
| code table `size + 1` | 31,226,139,271 |
| `firstFuel` (stage 1: 42,052 field cells by bounded rejection, the gadget bytes, the 792 fold joins, the key, serialisation) | 13,792,220,459 |
| `secondFuel` (stage 2, valid arm: parse, replay with the digit extraction of every replayed mask vector, opening, labels) | 11,903,632,071 |
| **total** `size + 1 + firstFuel + secondFuel` | **56,921,991,801 ≈ 2^35.73 ≤ 2^60** |

The stage-2 oracle traffic is 988,285 lazy queries (`Design.stage2Queries_eq`: 8,688 fold, 979,088
hash, the bridge hash and 508 whitening pads) and 452 hash programs (`stage2Programs_eq`). These
reconcile with the honest evaluator (`evaluator_queries`):

`988,285 + 452 + 508 (bit pads) + 46,228 (gadget) = 1,035,473`
...
| `Proof/Simulator/BigIntAccept.lean` | the sampler's acceptance at the designated vector: `t` uniform on the whole fibre `{t : enc + p^455·t < 2^115712}`, abort mass `< 2^-138` over 80 attempts |
| `Proof/Simulator/BigIntBound.lean` | the two large-numeral facts `preimageAccept_nat` and `preimageComplete_nat` (`decide +kernel`) |
| `Proof/Simulator/OpeningFree.lean` | the opening's 182 free designated coordinates (`Opening.nonCollectors`) |
...
Modules the two phases reworked, with the facts that changed:

| module | what it now proves |
|---|---|
| `Construction/PGS/Params.lean` | the width function `chunkWidthNat` (`[2, 5 × 32, 4 × 23]`), `chunkOffset_eq_sum`, the three-piece sums (`sum_chunkWidth`, `foldBase_eq`) |
| `Construction/ArgoMAC/Biquadratic.lean` | the four-element `Y` row: `evaluateEncodedY_raw` at every input, `evaluateEncodedY` on the curve |
| `Proof/Privacy/Phase3/Basic.lean` | the sampler's bias `sampleLane_etvDist_le` and the per-lane `laneDelta_le` |
| `Proof/Privacy/Phase3/MaskSwap.lean`, `GameSwap.lean` | the non-adaptive swap of every garbler mask vector (`maskSwap_etvDist_le`, `card_vectorSite = 5,584`) |
| `Proof/Privacy/Phase3/Opening.lean` | the collector solve with the `Y` collector's coefficient `x²` (`collectorSolve`, `collectorEquiv`, `collectorsOf_eq_solve`) |
...
| `Proof/Privacy/Phase3/PublicFirst/LawsGuess.lean` | the `12,245` coincidence events and `coincidenceError` |
| `Proof/Privacy/Phase3/Lazy/` | the abort sites `Lazy.AbortSite` and the refill at charge `0` (`IdealPerMask.maskSwapBound_of`) |
| `Proof/Simulator/Stage1Serialize.lean`, `Stage1Wire.lean`, `Stage1Match.lean` | the stage-1 serializer per chunk word, with the 256-bit last cell carrying the word's two zero bits |
| `Proof/Simulator/OpeningSolve.lean`, `OpeningLaw.lean` | the machine's `Y` solve `finishScaled` (`memSem_finishScaled`), equal to the abstract simulator's `collectorScale` (`solveWords_component`) |
