...
vector's cell of collector `c` (`collectorCell d c`: elements `5d + 1`, `5d + 3`, `5d + 4`,
`memSem_solveDigit`): P3's `collectorTargets`. The scale `s_c` inverts the collector's coefficient
in its row: `1` for `X` and `Z`, `(x · x)⁻¹` for `Y`, whose collector `rowY_cubic` rides on `x²`
(`Opening.finishScaled`, `solveWords`).
...
/-- The slot of collector `c` inside its digit: `1, 3, 4` (`rowX_x9`, `rowY_cubic`, `rowZ_x9`). -/
def collectorSlot (collector : Nat) : Nat := 2 * collector + 1 - collector / 2

/-- The designated vector's cell of collector `c` of digit `d`. -/
def collectorCell (digit collector : Nat) : Nat := designatedCell (5 * digit + collectorSlot collector)
...
macro "addr_arith" : tactic =>
  `(tactic| ((try simp only [Opening.rowCell, Opening.xCell, Opening.yCell, openRow, openPoint, openLambda, designatedCell, designatedBase, collectorCell, collectorSlot, fieldBase, curveCellCount, accBase, openBase, requestBase, reqX, reqY, reqTag0, reqQX, reqQY, tmpBase, tmpKappa, tmpJStar, hotLabelBase, designatedLabel, labelBase]) <;> omega))
...
/-- `RAM[target] = (RAM[row] − rAcc) · rF · rC⁻¹`: the `Y` row's finish, whose collector has
...
theorem run_rowX (digit : Nat) (rest : Prog) (memory : Memory) (x y kappaValue : BaseField)
    ...
    ∃ after, (Prog.seq (loadAt rAcc (Opening.rowCell digit 0))
        ...
        (Prog.seq (Opening.finishTarget (openRow digit) (designatedCell (5 * digit + 1))) rest)))))))).memSem
          memory = rest.memSem after ∧
      after.ram = Function.update memory.ram (word (designatedCell (5 * digit + 1)))
  ...
  obtain ⟨m8, run8, ram8, bits8, frame8⟩ := memSem_finishTarget (openRow digit) (designatedCell (5 * digit + 1)) m7
...
collector `rowY_cubic` enters the row with coefficient `x²`. The row reads `mixed · y`, then
`cubic · x²` (the running sum without `j*`), then `y8 · y` and `y10`. -/
theorem run_rowY (digit : Nat) (rest : Prog) (memory : Memory) (x y kappaValue : BaseField)
    (regsHold : SolveRegs memory x y kappaValue)
    (c0 c2 c3 c4 c5 vm vc v8 v10 target : BaseField)
    ...
    (hm : wordField (memory.ram (word (Opening.xCell (5 * digit + 2)))) = vm)
    (hc : wordField (memory.ram (word (Opening.xCell (5 * digit + 3)))) = vc)
    ...
    ∃ after, (Prog.seq (loadAt rAcc (Opening.rowCell digit 4))
        ...
        (Prog.seq (Opening.addScaled (Opening.xCell (5 * digit + 2)) rB)
        (Prog.seq (Opening.addScaled (Opening.xCell (5 * digit + 3)) rC)
        (Prog.seq (Opening.addScaled (Opening.yCell (3 * digit + 1)) rB)
        (Prog.seq (Opening.addCell (Opening.yCell (3 * digit + 2)))
        (Prog.seq (Opening.finishScaled (openRow digit + 1) (designatedCell (5 * digit + 3))) rest)))))))))).memSem
          memory = rest.memSem after ∧
      after.ram = Function.update memory.ram (word (designatedCell (5 * digit + 3)))
        (fieldWord ((target - (c0 + c2 * y + c3 * (x * y) + c4 * (x * x) + c5 * (y * y) + vm * y +
          vc * (x * x) + v8 * y + v10)) * kappaValue * (x * x)⁻¹)) ∧
      after.bits = memory.bits ∧ SolveRegs after x y kappaValue ∧
      ∀ index, index ≠ rAcc → index ≠ rSel → index ≠ rAddr →
        after.registers index = memory.registers index := by
  obtain ⟨ra, rb, rc, rd, re, rf⟩ := regsHold
  ...
  rw [memSem_pure_seq run5]
  have regs5 := regs4.frame frame5
  obtain ⟨m6, run6, ram6, bits6, acc6, frame6⟩ := step_addScaled _ rB m5 _ vm y (by decide)
    (by decide) acc5 (by rw [ram5, ram4, ram3, ram2, ram1]; exact hm) regs5.2.1
  rw [memSem_pure_seq run6]
  have regs6 := regs5.frame frame6
  obtain ⟨m7, run7, ram7, bits7, acc7, frame7⟩ := step_addScaled _ rC m6 _ vc (x * x) (by decide)
    (by decide) acc6 (by rw [ram6, ram5, ram4, ram3, ram2, ram1]; exact hc) regs6.2.2.1
  rw [memSem_pure_seq run7]
  have regs7 := regs6.frame frame7
  ...
  obtain ⟨m9, run9, ram9, bits9, acc9, frame9⟩ := step_addCell _ m8 _ v10 acc8
    (by rw [ram8, ram7, ram6, ram5, ram4, ram3, ram2, ram1]; exact h10)
  rw [memSem_pure_seq run9]
  have regs9 := regs8.frame frame9
  obtain ⟨m10, run10, ram10, bits10, frame10⟩ :=
    memSem_finishScaled (openRow digit + 1) (designatedCell (5 * digit + 3)) m9
  rw [memSem_pure_seq run10]
  have ramChain : m9.ram = memory.ram := by
    rw [ram9, ram8, ram7, ram6, ram5, ram4, ram3, ram2, ram1]
  refine ⟨m10, rfl, ?_, ?_, regs9.frame fun index h0' h5' h4' => frame10 index h0' h5' h4', ?_⟩
  · rw [ram10, acc9, regs9.2.2.2.2.2, regs9.2.2.1, ramChain, hw]
  · rw [bits10, bits9, bits8, bits7, bits6, bits5, bits4, bits3, bits2, m1Def]; rfl
  · intro index h0' h5' h4'
    rw [frame10 index h0' h5' h4', frame9 index h0' h5' h4', frame8 index h0' h5' h4',
      frame7 index h0' h5' h4', frame6 index h0' h5' h4', frame5 index h0' h5' h4',
      frame4 index h0' h5' h4', frame3 index h0' h5' h4', frame2 index h0' h5' h4',
...
theorem run_rowZ (digit : Nat) (rest : Prog) (memory : Memory) (x y kappaValue : BaseField)
    ...
    ∃ after, (Prog.seq (loadAt rAcc (Opening.rowCell digit 9))
        ...
        (Prog.seq (Opening.finishTarget (openRow digit + 2) (designatedCell (5 * digit + 4))) rest)))).memSem
          memory = rest.memSem after ∧
      after.ram = Function.update memory.ram (word (designatedCell (5 * digit + 4)))
  ...
  obtain ⟨m4, run4, ram4, bits4, frame4⟩ :=
    memSem_finishTarget (openRow digit + 2) (designatedCell (5 * digit + 4)) m3
  rw [memSem_pure_seq run4]
...
`κ · (W.y − row.y) · (x · x)⁻¹` for `Y`, whose collector `rowY_cubic` has coefficient `x²`. -/
...
theorem memSem_solveDigit (digit : Nat) (small : digit < 91) (memory : Memory)
  ...
  rw [runX]
  have readX : ∀ address, address < 2 ^ 256 → address ≠ designatedCell (5 * digit + 1) →
  ...
  obtain ⟨rowY, runY, ramY, bitsY, regsY, frameY⟩ := run_rowY digit _ rowX input.x input.y
    ...
    (gammaConst gamma 7) (gammaConst gamma 8) (values (.inl .rowY_mixed))
    (values (.inl .rowY_cubic)) (values (.inr .rowY_y8)) (values (.inr .rowY_y10)) target.y
    ...
    (by rw [readX _ (by addr_arith) (by addr_arith)]; exact hvx .rowY_mixed)
  ...
  rw [runY]
  have readY : ∀ address, address < 2 ^ 256 → address ≠ designatedCell (5 * digit + 1) →
      address ≠ designatedCell (5 * digit + 3) → rowY.ram (word address) = memory.ram (word address) := by
  ...
  · rw [ramZ, ramY, ramX, ramPre]
    ...
    rw [show collectorCell digit 0 = designatedCell (5 * digit + 1) from rfl,
      show collectorCell digit 1 = designatedCell (5 * digit + 3) from rfl,
      show collectorCell digit 2 = designatedCell (5 * digit + 4) from rfl]
    ...
    have eY : (target.y - (gammaConst gamma 4 + gammaConst gamma 5 * input.y +
        ...
        gammaConst gamma 8 * (input.y * input.y) + values (.inl .rowY_mixed) * input.y +
