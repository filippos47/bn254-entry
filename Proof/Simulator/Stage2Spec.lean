...
clamp, the `91` randomisers, the lifts, the `182` non-collectors, the collector solve and the
preimage sampler) followed by the `452` programs and a register clear. This file states what the
...
* `OpeningLaw`: the law of `openingFree.memSem`, read through `openView` (the `904` half cells the
...
/-- **The oracle-free part of the opening**: `Opening.program` without its `452` programs and
...
/-- The half cells the programs read: `halfCell i`, `i < 2 · 452`. -/
def halfCell (half : Nat) : Nat := BigInt.halfBase samplerBase BigInt.samplerLimbs + half

/-- Half `i` of `452` limbs: the low block of limb `⌊i / 2⌋` at even `i`, the high block at odd
...
structure OpeningPre (source : Stage1Source) (input : AffineInput) (target : Point)
  ...
  /-- The `pointY` lane's values, slots `458 .. 730`. -/
...
/-- The machine's view: the `904` half cells, `tmpJStar`, `E*`, the labels, the response stack. -/
