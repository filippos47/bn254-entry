...
theorem hot_source (draw : Stage1Draw) (s : Fin foldStepCount) :
    ...
    (drawSource draw).curveYHot.get s = BitVec.ofNat 128 (total 0 draw.2.2.1 (202 + s.val)) ∧
    (drawSource draw).pointXHot.get s = BitVec.ofNat 128 (total 0 draw.2.2.1 (404 + s.val)) ∧
    (drawSource draw).pointYHot.get s = BitVec.ofNat 128 (total 0 draw.2.2.1 (606 + s.val)) := by
