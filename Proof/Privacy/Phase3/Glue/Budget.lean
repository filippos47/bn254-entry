...
(lane, chunk, switch), `1,396` per lane.

| hop | term | value |
|---|---|---|
| `G0 → G0U` | `maskSwapError = 1396 · Σ_lane laneDelta lane`, with the per-vector bias `laneDelta` = `2^-142`, `2^-290`, `2^-262`, `2^-260` (pointX, pointY, curveX, curveY) | `≤ 1397/2^142 ≈ 2^-131.55` |
...
| `G1U → HW` | `stageOneHitError q₁ + exceptionalError + maskSwapError + coincidenceError`: `L2 = 4q₁/2^128`, plus the doubling-exception mass `182/(r−1)` (the gadget goes from real to lazy in this hop; P1d, `PublicFirst.gadget_counterShape_refutes`), plus one more mask swap, plus the label-coincidence allowance `2^16/2^128` of the designed sub-hop `G1U → G1U°` (P1j, `PublicFirst.LiftHop`). The mask swap is needed because off the curve `G1U` installs the system-A vectors with `G0U`'s uniform values while `HW` makes no call, so the adversary's own vectors are `sampleLane` of fresh hash answers (P1g; the middle game `M'` of `PublicFirst/MiddleOff.lean` carries the swap). | linear + `2^-112` |
...
| `I^U → I` | `maskSwapError + idealRefillError q₁`, `idealRefillError q₁ = 0` (lazy hash answers are exactly uniform) | `2^-131.55` |
...
terms: three mask swaps, `182/(r−1)`, `364/(r−1)`, the cutoff and the coincidence allowance; the
...
`Σ_c 2^{w_c} = 4 + 32·32 + 23·16 = 1,396` under the phase-5 profile (chunk widths
`[2, 5 × 32, 4 × 23]`, 56 chunks); `5,584` over the four lanes. It is stated as a numeral; the
`Params`-level identity `Σ_c 2^{w_c} = laneVectorCount` belongs with the mask swap, which counts
the vector sites. -/
def laneVectorCount : ℕ := 1396

/-- One copy of the global mask swap: every lane's `1,396` vectors, each at its lane's proof bias
`laneDelta` (`Security.Phase3.laneDelta_le`: `2^-142` pointX, `2^-290` pointY, `2^-262` curveX,
`2^-260` curveY). Its value is `≤ 1397/2^142 ≈ 2^-131.55` (`maskSwapError_le_pow`). -/
...
`maskSwapError` (`2^-131.55`) is too small for that, so it is its own term. -/
...
/-- **The mask-swap value**, `1396·(2^-142 + 2^-290 + 2^-262 + 2^-260) ≤ 1397/2^142 ≈ 2^-131.55`:
the three smaller lanes are each `≤ 2^-260 = 2^-142·2^-118`, and `3·1396·2^-118 ≤ 1`. -/
theorem maskSwapError_le_pow : maskSwapError ≤ 1397 / 2 ^ 142 := by
  ...
  have tail : (1 : ℝ) / 2 ^ 142 * (4188 * (1 / 2 ^ 118)) ≤ 1 / 2 ^ 142 * 1 :=
    mul_le_mul_of_nonneg_left (by norm_num) (by positivity)
  rw [maskSwapError_eq_sum, sum_lane, div_eq_mul_one_div (1397 : ℝ)]
...
theorem maskSwapError_le : maskSwapError * 2 ^ 100 ≤ 1 / 1000 := by
  calc maskSwapError * 2 ^ 100 ≤ 1397 / 2 ^ 142 * 2 ^ 100 :=
